...
Compared to fastqc.pl, this version can handle multiple fastq files simultaneously, and compressed files (*.gz) are also supported.
...
sub fastqc {
	...
	my ($filein, $fileout);
	if ($input =~ /\.gz$/) {
		open $filein, "gunzip -dc $dir/$input |" or die "Open fail: $!";
	} else {
		open $filein, "<$dir/$input" or die "Open fail: $!";
	}
	open $fileout, ">$dir/$input.qc2" or die "Create fail: $!";
